...
import com.example.parkingsystem.classes.SlotNoInfo;
...
public class AddPositionActivity extends AppCompatActivity implements OnMapReadyCallback {
    ...
    private void attachListeners() {
        db.getReference().child("ParkingAreas").child(auth.getCurrentUser().getUid()).addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                parkingArea = snapshot.getValue(ParkingArea.class);
                if (parkingArea != null) {
                    Log.e(String.valueOf(AddPositionActivity.this.getClass()), "Fetch Parking Details");
                    areaNameText.setText(String.valueOf(parkingArea.name));
                    areaNameText.setSelection(areaNameText.getText().length());
                    amount2Text.setText(String.valueOf(parkingArea.amount2));
                    amount2Text.setSelection(amount2Text.getText().length());
                    amount3Text.setText(String.valueOf(parkingArea.amount3));
                    amount3Text.setSelection(amount3Text.getText().length());
                    amount4Text.setText(String.valueOf(parkingArea.amount4));
                    amount4Text.setSelection(amount4Text.getText().length());
                    totalSlotsText.setText(String.valueOf(parkingArea.totalSlots));
                    totalSlotsText.setSelection(totalSlotsText.getText().length());
